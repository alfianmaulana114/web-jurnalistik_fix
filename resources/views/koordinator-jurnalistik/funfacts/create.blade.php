...
@section('header', 'Tambah Funfact')

@section('content')
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h3 class="text-2xl font-medium text-gray-800 mb-6">Tambah Funfact Baru</h3>

        <form action="{{ route('koordinator-jurnalistik.funfacts.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="isi" id="isi" rows="6" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent @error('isi') border-red-500 @enderror" placeholder="Tulis isi funfact di sini..." required>{{ old('isi') }}</textarea>
                    @error('isi')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-8 flex justify-end space-x-4">
                <a href="{{ route('koordinator-jurnalistik.funfacts.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors">Batal</a>
                <button type="submit" id="submitBtn" class="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                    <i class="fas fa-save mr-2"></i>Simpan Funfact
                </button>
            </div>
        </form>
    </div>
