...
@section('title', 'Buat Content Baru')
@section('header', 'Buat Content Baru')

@section('content')
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="rounded-xl border border-[#D8C4B6]/40 bg-white shadow-sm p-6">
        <h3 class="text-2xl font-medium text-[#1b334e] mb-6">Buat Content Baru</h3>
        
        <form action="{{ route('koordinator-humas.contents.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2" data-lang-section="id">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-[#D8C4B6]/40 rounded-lg focus:ring-[#f9b61a] focus:border-[#f9b61a] @error('judul_id') border-red-500 @enderror" 
                           id="judul_id" name="judul_id" value="{{ old('judul_id', $selectedDesignTitle ?? '') }}" 
            ...
            <div class="mt-8 flex justify-end space-x-4">
                <a href="{{ route('koordinator-humas.contents.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
                    Batal
                </a>
                <button type="submit" id="submitBtn" class="px-6 py-3 bg-[#1b334e] text-white rounded-lg hover:bg-[#1b334e]/90 transition-colors">
                    <i class="fas fa-save mr-2"></i>Simpan Content
                </button>
            </div>
        </form>
    </div>
