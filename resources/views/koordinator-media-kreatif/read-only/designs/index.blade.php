...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg border border-[#D8C4B6]/40 shadow-sm hover:shadow-md transition-all p-6">
        <form method="GET" action="{{ route(($routePrefix ?? 'koordinator-media-kreatif.view').'.designs.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route(($routePrefix ?? 'koordinator-media-kreatif.view').'.designs.index') }}" class="px-4 py-2 bg-white text-[#1b334e] border border-[#D8C4B6]/40 rounded-lg font-semibold text-xs uppercase tracking-widest hover:bg-[#f9b61a]/10 focus:outline-none transition-all duration-150">
                        ...
                        @foreach($designs as $design)
                            <tr class="hover:bg-[#f9b61a]/5 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a href="{{ route(($routePrefix ?? 'koordinator-media-kreatif.view').'.designs.show', $design) }}" class="font-medium hover:text-[#f9b61a]">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    @if($design->berita)
                                        <a href="{{ route(($routePrefix ?? 'koordinator-media-kreatif.view').'.news.show', $design->berita->id) }}" class="text-[#1b334e] hover:text-[#f9b61a]">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route(($routePrefix ?? 'koordinator-media-kreatif.view').'.designs.show', $design) }}" class="text-[#1b334e] hover:bg-[#f9b61a]/10 p-2 rounded-lg transition-all" title="Lihat Detail">
