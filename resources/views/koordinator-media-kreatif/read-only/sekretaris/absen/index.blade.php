...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <form method="GET" action="{{ route('koordinator-media-kreatif.view.sekretaris.absen.index') }}" class="flex items-center gap-2">
                ...
                <a href="{{ route('koordinator-media-kreatif.view.sekretaris.absen.index') }}" class="px-3 py-2 bg-gray-200 text-gray-700 rounded-md text-sm">Reset</a>
            </form>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($meetings as $m)
            <a href="{{ route('koordinator-media-kreatif.view.sekretaris.absen.index', ['notulensi_id' => $m->id]) }}" class="group block rounded-lg border border-gray-200 bg-white p-4 shadow-sm hover:shadow-md transition">
