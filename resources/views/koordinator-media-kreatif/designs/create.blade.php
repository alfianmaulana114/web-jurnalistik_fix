...
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="rounded-xl border border-[#D8C4B6]/40 bg-white shadow-sm p-6">
        <h3 class="text-2xl font-medium text-[#1b334e] mb-6">Tambah Desain Media</h3>

        <form action="{{ route('koordinator-media-kreatif.designs.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" id="judul_auto" class="w-full px-3 py-2 border border-[#D8C4B6]/40 rounded-lg bg-gray-100 focus:ring-[#f9b61a] focus:border-[#f9b61a]" value="{{ $selectedNewsTitle ?? '' }}" placeholder="Pilih berita untuk menampilkan judul" disabled>
                ...
                <div class="col-span-2">
                    ...
                    <input type="url" name="media_url" id="media_url" value="{{ old('media_url') }}" class="w-full px-3 py-2 border border-[#D8C4B6]/40 rounded-lg focus:ring-[#f9b61a] focus:border-[#f9b61a]" placeholder="https://...">
                    @error('media_url')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="catatan" id="catatan" rows="4" class="w-full px-3 py-2 border border-[#D8C4B6]/40 rounded-lg focus:ring-[#f9b61a] focus:border-[#f9b61a]" placeholder="Catatan tambahan">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-8 flex justify-end space-x-4">
                <a href="{{ route('koordinator-media-kreatif.designs.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">Batal</a>
