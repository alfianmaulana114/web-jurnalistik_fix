...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" class="flex gap-3 items-end" action="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.brief-humas.index') }}">
            <div class="flex-1">
            ...
            <a href="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.brief-humas.index') }}" class="px-4 py-2 border rounded-md text-sm">Reset</a>
        </form>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900">
                        <a href="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.brief-humas.show', $briefHumas) }}" class="font-medium text-[#1b334e] hover:text-[#f9b61a]">{{ $briefHumas->judul }}</a>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-600">{{ Str::limit($briefHumas->catatan, 80) }}</td>
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.brief-humas.show', $briefHumas) }}" class="text-[#1b334e] hover:bg-[#f9b61a]/10 p-2 rounded-lg transition-all" title="Lihat Detail">
