...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg border border-[#D8C4B6]/40 shadow-sm hover:shadow-md transition-all p-6">
        <form method="GET" action="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.users.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.users.index') }}" class="inline-flex items-center px-4 py-2 bg-white text-[#1b334e] border border-[#D8C4B6]/40 rounded-lg font-semibold text-xs uppercase tracking-widest hover:bg-[#f9b61a]/10 focus:outline-none transition-all duration-150">
                        ...
                        @foreach($users as $user)
                            <tr class="hover:bg-[#f9b61a]/5 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route(($routePrefix ?? 'koordinator-litbang.view').'.users.show', $user) }}" class="text-[#1b334e] hover:bg-[#f9b61a]/10 p-2 rounded-lg transition-all" title="Lihat Detail">
