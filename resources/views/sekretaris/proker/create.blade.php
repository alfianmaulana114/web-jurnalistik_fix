...
<div class="container mx-auto px-4 py-6 max-w-4xl">
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <form action="{{ route('sekretaris.proker.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="col-span-2">
                    ...
                    <input type="text" name="nama_proker" id="nama_proker" value="{{ old('nama_proker') }}" required
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="catatan" id="catatan" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent @error('catatan') border-red-500 @enderror">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="border-t border-gray-200 pt-6 mt-6">
                ...
                <div id="committee-container" class="space-y-4">
                    <div class="committee-item grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border border-gray-200 rounded-lg">
                        <div>
                            ...
                            <select name="committees[0][user_id]"
                        ...
                        <div>
                            ...
                            <input type="text" name="committees[0][jabatan]" placeholder="Ketua, Sekretaris, dll."
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tugas</label>
                            <input type="text" name="committees[0][tugas]" placeholder="Deskripsi tugas"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-400 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <button type="button" id="add-committee"
            ...
            <div class="flex justify-end space-x-4 mt-6">
                <a href="{{ route('sekretaris.proker.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors">
                    Batal
                </a>
...
<script>
    ...
    document.getElementById('add-committee').addEventListener('click', function() {
        ...
        newCommittee.className =
            'committee-item grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border border-gray-200 rounded-lg';
