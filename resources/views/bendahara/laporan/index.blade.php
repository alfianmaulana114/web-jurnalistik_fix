...
function viewReport(type) {
    ...
    let url = '';
    if (type === 'kas-anggota') {
        url = `/bendahara/laporan/kas-anggota?tahun=${tahun}`;
    } else if (type === 'pemasukan') {
        url = `/bendahara/pemasukan?tahun=${tahun}`;
    } else if (type === 'pengeluaran') {
        url = `/bendahara/pengeluaran?tahun=${tahun}`;
    } else {
        url = `/bendahara/laporan?tahun=${tahun}&periode=${periode}`;
...
function exportReport(type) {
    ...
    // Build export URL (controller exportExcel expects type, tahun, periode; bulan opsional)
    let url = `/bendahara/laporan/export-excel?type=${type}&tahun=${tahun}&periode=${periode}`;
    if (periode === 'custom' && tanggalMulai && tanggalSelesai) {
        url += `&tanggal_mulai=${encodeURIComponent(tanggalMulai)}&tanggal_selesai=${encodeURIComponent(tanggalSelesai)}`;
    }
...
function exportAllReports() {
    ...
    let url = `/bendahara/laporan/export-excel?type=total-saldo&tahun=${tahun}&periode=${periode}`;
    if (periode === 'custom' && tanggalMulai && tanggalSelesai) {
        url += `&tanggal_mulai=${encodeURIComponent(tanggalMulai)}&tanggal_selesai=${encodeURIComponent(tanggalSelesai)}`;
    }
