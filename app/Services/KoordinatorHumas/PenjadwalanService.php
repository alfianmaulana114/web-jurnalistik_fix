...
/**
 * Service untuk Manajemen Penjadwalan Koordinator Humas
 * 
 * Service ini menangani semua logika bisnis terkait penjadwalan anggota humas.
...
class PenjadwalanService
{
     ...
     * @return array Data penjadwalan dan anggota humas
     */
    public function index(Request $request): array
    {
        ...
        // Ambil semua anggota humas
        $anggotaHumas = User::whereIn('role', [
            User::ROLE_ANGGOTA_HUMAS,
            User::ROLE_KOORDINATOR_HUMAS
        ])->orderBy('name')->get();

        // Filter penjadwalan hanya untuk anggota humas
        $humasUserIds = $anggotaHumas->pluck('id');
        
        $penjadwalan = Penjadwalan::with(['user'])
            ->whereIn('user_id', $humasUserIds)
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('koordinator-humas.penjadwalan.index', [
            'bulan' => $bulan,
            'tahun' => $tahun,
    ...
    private function validatePenjadwalan(Request $request, ?int $id = null): array
    {
        ...
        // Validasi tambahan untuk memastikan user adalah anggota humas
        $user = User::find($validated['user_id']);
        if (!$user || !in_array($user->role, [User::ROLE_ANGGOTA_HUMAS, User::ROLE_KOORDINATOR_HUMAS])) {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'user_id' => ['User yang dipilih harus anggota atau koordinator humas.'],
            ]);
        }
